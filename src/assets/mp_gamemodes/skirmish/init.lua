local gamemode = {}

gamemode.id = "skirmish"
gamemode.name = "Skirmish"
gamemode.desc = "Test your mettle against AI or other players with teams."
gamemode.start_level = "1"
gamemode.configurable = true
gamemode.configurable_team = true
gamemode.configurable_race = true
gamemode.configurable_diff = true
gamemode.weight = 100
gamemode.guide_text = [[Welcome to the IEE war game simulator.

Configure your players to prepare your match.]]

return gamemode
